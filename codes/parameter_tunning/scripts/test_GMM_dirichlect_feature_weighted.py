...
def test_parameter_combinations_dynamic(fits_path, start_velocity, end_velocity, seeds,
                                        ...
                                        tolerance_range=[0.001],
                                        vel_importance=[1, 2, 4, 6],
                                        bright_importance=[1, 2, 4, 6],
                                        xy_importance=[1, 2, 4, 6],
                                        group_num=-1, ofile=""):
    # Prepare data
    ...
    param_combinations = itertools.product(tolerance_range, weight_range, pixel_value_range, xy_range)
    ...
    # Iterate over all combinations and run tests
...
def calculate_params_and_error(params, scaled_dataset, scaled_seeds, original_dataset, group_num, alpha, beta, ofile):
    ...
    tolerance, w, v, xy = params
    print("RUNNING FOR: ",tolerance)

    start_time = time.time() 

    labels = cluster_with_GMM(scaled_dataset, len(scaled_seeds), tolerance,
                               alpha, beta, w, v, xy)
...
def cluster_with_GMM(data, n_components, DP=5, alpha=0.5, beta=0.3, w=1, v=1, xy=1):
    """
...
def post_process_data_optimized(df):
    ...
    pixel_values = df['pixel_value'].tolist()
    ...
    for i in range(len(labels)):
        ...
        if isinstance(label, (int, np.integer)):
            ...
            if len(label) == 1 and isinstance(label[0], (int, np.integer)):
                # Take the integer as the new label
                new_label = label[0]
                ...
                if all(isinstance(item, tuple) and len(item) == 2 for item in label):
                    ...
                    new_pixel_values = np.array(pixel_value) * np.array(probs)
                    ...
                    # Handle cases where label is not in the proper format
    ...
    # Create the pixel sums DataFrame
    pixel_sums_df = redistributed_df.groupby(['label', 'velocity'])['pixel_value'].sum().reset_index()

    min_pixel_sum = pixel_sums_df['pixel_value'].min()
    max_pixel_sum = pixel_sums_df['pixel_value'].max()
    norm_range = [0, 3.2428885]
    pixel_sums_df['normalized_pixel_sum'] = norm_range[0] + ((pixel_sums_df['pixel_value'] - min_pixel_sum) * (norm_range[1] - norm_range[0]) / (max_pixel_sum - min_pixel_sum))
...
tolerance_values = [0.01,0.1,1,3,5,10,15,30,50,100,150,200,250,500]
