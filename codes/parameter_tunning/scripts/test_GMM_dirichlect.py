...
def test_parameter_combinations_dynamic(fits_path, start_velocity, end_velocity, seeds,
    ...
    print(len(tolerance_range))

    # Iterate over all combinations and run tests
    for combination in tolerance_range:
...
def calculate_params_and_error(params, scaled_dataset, scaled_seeds, original_dataset, group_num, alpha, beta, ofile):
    ...
    labels = cluster_with_GMM(scaled_dataset, len(scaled_seeds), tolerance, alpha, beta)
...
def cluster_with_GMM(data, n_components, DP=5, alpha=0.5, beta=0.3):
    """
...
tolerance_values = []
tolerance_values += list(np.arange(0.01,1,0.1))
tolerance_values += list(np.arange(1,10,1))
tolerance_values += list(np.arange(10,50,5))
tolerance_values += list(np.arange(50,200,10))
tolerance_values += list(np.arange(200,500,50))
