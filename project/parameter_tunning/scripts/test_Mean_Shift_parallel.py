...
def worker_function(combination, scaled_dataset, scaled_seeds, original_dataset, group_num, epsilon, max_iter, weighting_method, alpha, beta, seeds, ofile):
    # Call the calculate_params_and_error function with the lock
    calculate_params_and_error(combination, scaled_dataset, scaled_seeds, original_dataset, group_num, epsilon, max_iter, weighting_method, alpha, beta, seeds, ofile)
...
def test_parameter_combinations_dynamic(fits_path, start_velocity, end_velocity, seeds,
    ...
    with multiprocessing.Pool(processes=8) as pool:
        ...
        for i in param_combinations_with_others:
            result = pool.apply_async(worker_function, args=(i, scaled_dataset, scaled_seeds, original_dataset, group_num, epsilon, max_iter, weighting_method, alpha, beta, seeds, ofile))
...
def calculate_params_and_error(params, scaled_dataset, scaled_seeds, original_dataset, group_num, epsilon, max_iter, weighting_method, alpha, beta,seeds,ofile):
    ...
    print(f"ALL ITERATION took {total_time} seconds to run.")
...
def mean_shift_clustering(X, seeds, bandwidths, epsilon=1e-3, max_iter=10, fweight_velocity=1, fweight_pixel_value=1, fweight_xy=1):
    """
    Modified Mean Shift Clustering Algorithm.
...
output_file_name = "output_MS_13CO_"+ suffix_list[gnum] +".txt"
