...
def worker_function(combination, scaled_dataset, scaled_seeds, original_dataset, group_num, alpha, beta, ofile):
    # Call the calculate_params_and_error function with the lock
    calculate_params_and_error(combination, scaled_dataset, scaled_seeds, original_dataset, group_num, alpha, beta, ofile)
...
def test_parameter_combinations_dynamic(fits_path, start_velocity, end_velocity, seeds,
                                        ...
                                        scaling_method='standard', alpha=0.7, beta=0.2,
                                        tolerance_range=[0.001], weights_list=[None],
                                        group_num=-1, ofile=""):
    # Prepare data

    start_time = time.time()  

    scaled_dataset, scaler, original_dataset = prepare_data(fits_path, start_velocity, end_velocity, use_otsu, otsu_mult_factor, scaling_method)
    ...
    all_combinations = itertools.product(weights_list, tolerance_range)
    ...
    num_processes = multiprocessing.cpu_count()  # or any number you choose
    ...
    with multiprocessing.Pool(processes=8) as pool:
        ...
        # Iterating over param_combinations_with_others
        for i in all_combinations:
            result = pool.apply_async(worker_function, args=(i, scaled_dataset, scaled_seeds, original_dataset, group_num, alpha, beta, ofile))
...
def calculate_params_and_error(params, scaled_dataset, scaled_seeds, original_dataset, group_num, alpha, beta, ofile):
    
    weights, tolerance = params
    print("RUNNING FOR: ",weights,tolerance)

    start_time = time.time()  

    labels = cluster_with_GMM(scaled_dataset, len(scaled_seeds), tolerance, weights, scaled_seeds, alpha, beta)
...
def generate_weight_combinations(parts, total=1.0, step=0.05):
    if parts == 1:
        # Only one way to have a single part that sums to the total
        return [[total]]
    else:
        combinations = []
        current_value = 0.05
        while current_value <= total:
            # Generate combinations for the remaining parts
            for sub_combination in generate_weight_combinations(parts - 1, total - current_value, step):
                combinations.append([current_value] + sub_combination)
            current_value += step
        # Filter combinations where the sum of parts is exactly the total
        return [comb for comb in combinations if sum(comb) == total]
...
all_weights_comb = generate_weight_combinations(len(seeds_list[gnum]))
tolerance_values = [0.001,0.005, 0.01, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3]

    
suffix_list = ["20","117-114","109-100","80-56","47-35"]
output_file_name = "output_GMM_13CO_"+ suffix_list[gnum] +".txt"
...
test_parameter_combinations_dynamic(fits_path, intervals_list[gnum][0],
                                    ...
                                    group_num=gnum,alpha=0.7, beta=0.2,
                                    tolerance_range=tolerance_values, weights_list=all_weights_comb,
